...
export function DashboardSidebar({
  ...
  return (
    <TooltipProvider delayDuration={150}>
      <aside
        className={`transition-[width] duration-300 ease-in-out ${collapsed ? "w-14" : "w-60"
      ...
      >
        <div className="flex h-full flex-col overflow-hidden rounded-xl border border-border/70 bg-background/85 backdrop-blur-xl shadow-sm">
          ...
          <div
            className={`flex h-11 shrink-0 items-center border-b border-border/60 ${collapsed ? "justify-center px-2" : "justify-between px-3"
              }`}
          >
            {!collapsed && (
            ...
            {filteredSidebarItems.map((item) => {
              ...
              const btn = (
                <Button
                  ...
                  className={`group w-full justify-start rounded-lg h-9 px-2.5 text-sm font-medium transition-all ${active
                ...
                >
                  <Link href={item.href}>
                    <Icon
                      className={`h-[1.1rem] w-[1.1rem] shrink-0 transition-colors ${active
                          ? "text-primary"
                          : "text-muted-foreground group-hover:text-foreground"
                        }`}
                    />
                    <span
                      className={`overflow-hidden whitespace-nowrap transition-all duration-300 ${collapsed ? "w-0 ml-0 opacity-0" : "w-full ml-2.5 opacity-100"
                        }`}
                    >
                      {item.label}
                    </span>
